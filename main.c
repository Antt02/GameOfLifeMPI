...
int main(int argc, char** argv)
{
	...
	while (quit==false && (EndTime<0 || Iteration<EndTime)) 
	{
        ...
        if (rank==0) { //FIRST NODE
            ...
            MPI_Send(&myBoard->cell_state[rows-1], 1, boardRow, 1, 1, MPI_COMM_WORLD);
            //recieve my upper from previous
            MPI_Recv(&myBoard->upper, 1, boardRow, size-1, 1, MPI_COMM_WORLD, &st);
            //send my first row -> under for the previous
            MPI_Send(&myBoard->cell_state[0], 1, boardRow, size-1, 2, MPI_COMM_WORLD);
            //recieve my under from next
            MPI_Recv(&myBoard->under, 1, boardRow, 1, 2, MPI_COMM_WORLD, &st);
        }else if(rank==size-1){ //LAST NODE
            //recieve my upper from previous
            MPI_Recv(&myBoard->upper, 1, boardRow, size-2, 1, MPI_COMM_WORLD, &st);
            //send my last row -> upper for the next
            MPI_Send(&myBoard->cell_state[rows-1], 1, boardRow, 0, 1, MPI_COMM_WORLD);
            //recieve my under from next
            MPI_Recv(&myBoard->under, 1, boardRow, 0, 2, MPI_COMM_WORLD, &st);
            //send my first row -> under for the previous
            MPI_Send(&myBoard->cell_state[0], 1, boardRow, size-2, 2, MPI_COMM_WORLD);
        }else{ //INTERMEDIUM NODE
            //recieve my upper from previous
            MPI_Recv(&myBoard->upper, 1, boardRow, rank-1, 1, MPI_COMM_WORLD, &st);
            //send my last row -> upper for the next
            MPI_Send(&myBoard->cell_state[rows-1], 1, boardRow, rank+1, 1, MPI_COMM_WORLD);
            //recieve my under from next
            MPI_Recv(&myBoard->under, 1, boardRow, rank+1, 2, MPI_COMM_WORLD, &st);
            //send my first row -> under for the previous
            MPI_Send(&myBoard->cell_state[0], 1, boardRow, rank-1, 2, MPI_COMM_WORLD);
        }
		MPI_Comm_rank(MPI_COMM_WORLD, &rank);
		myBoard->rank=rank;
